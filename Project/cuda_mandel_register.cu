...
__global__ void mandelbrot(char *colors, char* pixels, int height, int width, int max_iter) {
    ...
    double x = (double)(global_index % width);
    double y = (double)(global_index / width);

    double f_width = (double)width, f_height = (double)height;


    double c_re = (x - f_height / 2.0) * 4.0 / f_height;
    double c_im = (y - f_height / 2.0) * 4.0 / f_height;
