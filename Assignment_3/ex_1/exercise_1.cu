...
{
  long long x = threadIdx.x + blockDim.x * blockIdx.x;
  long long y = threadIdx.y + blockDim.y * blockIdx.y;

  long long start_offset = (x + y * width);
  long long start_offset_rgb = start_offset * 3;

  if (start_offset < width * height) {
    image_out[start_offset] = image[start_offset_rgb] * 0.0722f + 
                              image[start_offset_rgb + 1] * 0.7152f +
                              image[start_offset_rgb + 2] * 0.2126f;  
  }
}
...
__global__ void gpu_gaussian(int width, int height, float *image, float *image_out)
{
    ...
    int shared_index_x = 1 + threadIdx.x;
    int shared_index_y = 1 + threadIdx.y;

    // Remember this to not have to recalcualte it
    int global_index = index_x + index_y * width;
    int local_index = shared_index_x + shared_index_y * BLOCK_SIZE_SH;

    // First each thread transfer its pixel
    sh_block[local_index] = image[global_index];

    // Now all boundary threads need to transfer their additional pixels
    // If you are a left boundary you must transfer a left pixel, a right boundary a right pixel, a top boundary a top pixel
    // and a bottom boundary a bottom pixel -- corner boundaries must also transfer a corner pixel -- they will have to transfer
    // 3 additional pixels in total!

    bool is_local_top = shared_index_y == 1, 
         is_local_left = shared_index_x == 1, 
         is_local_right = shared_index_x == BLOCK_SIZE_SH - 2, 
         is_local_bottom = shared_index_y == BLOCK_SIZE_SH - 2,
         is_global_top = index_y <= 0,
         is_global_left = index_x <= 0,
         is_global_right = index_x >= width - 1,
         is_global_bottom = index_y >= height - 1;

    // if you're a left boundary
    if (is_local_left == 1 && !is_global_left) 
      sh_block[local_index - 1] = image[global_index - 1];

    // if you're a right boundary
    if (is_local_right && !is_global_right) 
      sh_block[local_index + 1] = image[global_index + 1];

    // if you're a top boundary
    if (is_local_top && !is_global_top) 
      sh_block[local_index - BLOCK_SIZE_SH] = image[global_index - width];

    // if you're a bottom boundary
    if (is_local_bottom && !is_global_bottom) 
      sh_block[local_index + BLOCK_SIZE_SH] = image[global_index + width];

    // And then we have 4 more edge cases for all of the corner boundaries

    // If is top-left corner - but not global top-left
    if (is_local_left && is_local_top && !is_global_left && !is_global_top) 
      sh_block[local_index - BLOCK_SIZE_SH - 1] = image[global_index - width - 1];

    // If is bottom-left corner - but not global bottom-left
    if (is_local_left && is_local_bottom && !is_global_left && !is_global_bottom) 
      sh_block[local_index + BLOCK_SIZE_SH - 1] = image[global_index + width - 1];

    // If is top-right corner - but not global top-right
    if (is_local_right && is_local_top && !is_global_right && !is_global_top) 
      sh_block[local_index - BLOCK_SIZE_SH + 1] = image[global_index - width + 1];

    // If is bottom-right corner - but not global bottom-right
    if (is_local_right && is_local_bottom && !is_global_right && !is_global_bottom) 
      sh_block[local_index + BLOCK_SIZE_SH + 1] = image[global_index + width + 1];
    ...
    if (index_x < (width - 2) && index_y < (height - 2))
    {
        ...
        int offset_t = local_index - BLOCK_SIZE_SH - 1;
        
        image_out[offset] = applyFilter(sh_block + offset_t, 
